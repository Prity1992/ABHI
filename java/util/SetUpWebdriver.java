	...
	public static WebDriver setupWebDriver(String driverType, String testScenario_Id) throws Exception {
		try {
			...
			case "Desktop":
				...
				/*
				 * options.addArguments("--proxy-server=socks5://127.0.0.1:4000"); options.
			...
			case "Chrome":
				/*
				 * ChromeOptions optionsc = new ChromeOptions(); capabilities = new
				 * DesiredCapabilities(); optionsc.setAcceptInsecureCerts(true); String proxy =
				 * "--proxy-server=socks5://127.0.0.1:4000"; optionsc.addArguments(proxy);
				 * capabilities.setCapability(ChromeOptions.CAPABILITY, getChromeOptions());
				 * capabilities.setCapability(CapabilityType.ACCEPT_SSL_CERTS, true);
				 * capabilities.setCapability(ChromeOptions.CAPABILITY, getChromeOptions());
				 * capabilities.setCapability(ChromeOptions.CAPABILITY, optionsc);
				 * capabilities.setJavascriptEnabled(true);
				 * capabilities.setBrowserName("chrome"); optionsc.merge(capabilities);
				 * setupDriver("webdriver.chrome.driver", "chromedriver");
				 * capabilities.setCapability(CapabilityType.ACCEPT_SSL_CERTS, true);
				 * capabilities.setCapability(ChromeOptions.CAPABILITY, getChromeOptions());
				 ...
				 */
				  ...
				  capabilities = new DesiredCapabilities();
				  options1.addArguments("--proxy-server=socks5://127.0.0.1:4000");
			...
			case "Ie":
				...
				options2.setBinary(getAppPath() + getAppExeName());
